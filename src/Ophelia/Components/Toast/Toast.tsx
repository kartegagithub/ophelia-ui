...
const Toast: React.FC<ToastProps> = ({ 
  ...
  return (
    <div id={id} className={Theme?.Class} role="alert">
      {image && <div className={Theme?.ImageClass}>
        {getImageComponent(image)}
      </div>}
      <div className={Theme?.TextClass}>{text}</div>
      {showCloseButton && <button onClick={() => setOpen(!open)} type="button" className={Theme?.CloseButtonClass} data-dismiss-target={"#" + id}>
