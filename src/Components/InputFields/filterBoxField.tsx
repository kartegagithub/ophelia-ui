...
> {
  ...
  promiseOptions = (input: string, callback: (options: any[]) => void) => {
    if (
      ...
      this.Searching = true;
      var result = this.props.remoteDataSource.CallFunction(input, 0, 0);
      result.then((data) => {
        var valueProp =
          this.props.remoteDataSource?.ValueProp ??
        ...
        data = data.map((item: any) => {
          return { label: item[displayProp], value: item[valueProp] };
        });
        callback(data);
  ...
  renderInput() {
    ...
    return (
      <AsyncSelect
        isMulti={this.props.isMulti ?? this.props.multiple}
