...
export function getImageComponent(
  elem: string | React.JSX.Element | Function | IconProps | any | undefined,
  props?: any,
  funcParams?: any
): React.JSX.Element {
  if (!elem) return <></>;
  ...
  if (typeof elem === "string") {
    if (
      ...
      if ((theme.Icons as any)[elem]) return (theme.Icons as any)[elem];
        ...
        var imageComponent = theme.Icons.getIconSvg(
          elem,
          props?.size ?? theme.Icons.DefaultSize,
          props.className,
          props.color,
          props.fill,
          props.ext1,
        ...
        if (typeof imageComponent === "string")
          return (
            <RawHTML clearHtml={false} sanitize={false} html={imageComponent} />
          );
