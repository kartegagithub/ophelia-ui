...
const Tabs: React.FC<{
  ...
  const TabHeaderButtonClass =
    (Theme?.Types as any)[type] ?? Theme?.TabHeaderButtonClass;
  const SelectedTabHeaderButtonClass =
    (Theme?.SelectedTypes as any)[type] ?? Theme?.SelectedTabHeaderButtonClass;
  ...
  return (
    <div
      className={`${Theme?.RootClass} ${tabPosition == "vertical" ? "flex-row" : "flex-col"}`}
      key={id}
    >
      {tabs.length > 1 && (
        <ul
          className={`${Theme?.TabHeaderClass} ${tabPosition == "vertical" && "flex-col"}`}
          key="nav-tabs"
        >
          {tabs
            ...
            .map((tab, i) => {
              ...
              return (
                <li
                  className={Theme?.TabHeaderButtonContainerClass}
                  role="presentation"
                  key={i}
                  onClick={() => {
      ...
      <div className={Theme?.TabContentClass}>
        {tabs.map((tab: any, i) => {
          ...
          return (
            <Tab theme={Theme} key={i} active={selected} {...otherProps}></Tab>
          );
        })}
      </div>
    </div>
